...
class TestTryStore:
    def test_add_and_list_all(self, tmp_path: Path):
        store = TryStore(tmp_path / "records.json")
        record = make_record()
        store.add(record)
    ...
    def test_persists_across_instances(self, tmp_path: Path):
        ...
        TryStore(path).add(make_record(id="r1"))
        assert len(TryStore(path).list_all()) == 1
    ...
    def test_creates_parent_directory_if_not_exists(self, tmp_path: Path):
        ...
        TryStore(nested).add(make_record())
        assert nested.exists()

    def test_roundtrip_preserves_all_fields(self, tmp_path: Path):
        path = tmp_path / "records.json"
        record = make_record()
        TryStore(path).add(record)
        loaded = TryStore(path).list_all()[0]
