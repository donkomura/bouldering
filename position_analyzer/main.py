...
def parse_args() -> argparse.Namespace:
    ...
    _ = parser.add_argument(
        "-i", "--input", required=True, help="入力動画 (例: IMG_1540.MOV)"
    )
    _ = parser.add_argument(
        "-o", "--output", default="output.mp4", help="出力動画 (例: result.mp4)"
    )
    _ = parser.add_argument(
        "-t", "--trail", type=int, default=120, help="軌跡を残すフレーム数"
    )
    _ = parser.add_argument(
        "-k", "--keep-trail", action="store_true", help="軌跡を動画中ずっと残す"
    )
    _ = parser.add_argument(
        "-m",
        "--model",
        default=DEFAULT_MODEL_PATH,
        help="PoseLandmarker モデル (.task) のパス",
    )
    _ = parser.add_argument(
        "--gpu", action="store_true", help="GPU を使用して推論を実行する"
    )
